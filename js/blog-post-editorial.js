...
(function(){
  ...
  function readingMinutes(markdown){
    ...
    if(typeof Intl!=='undefined'&&Intl.Segmenter){
      try{
        ...
        words=[...segmenter.segment(source)].filter(part=>part.isWordLike).length;
      }catch(_err){words=0;}
    }
    if(!words){
      words=locale==='th'
        ? Math.max(1,Math.round(source.replace(/\s+/g,'').length/5))
        : source.split(/\s+/).filter(Boolean).length;
    }
    const rate=locale==='th'?250:210;
    return Math.max(1,Math.round(words/rate));
  }

  async function copyText(value){
    if(navigator.clipboard&&window.isSecureContext){
      try{await navigator.clipboard.writeText(value);return true;}catch(_err){}
    }
  ...
  function buildToc(){
    const content=$('post-content'),toc=$('post-toc'),rail=$('post-rail');
    if(!content||!toc)return;
    const heads=[...content.querySelectorAll('h2')];
    if(!heads.length){if(rail)rail.hidden=true;toc.innerHTML='';return;}
    if(rail)rail.hidden=false;
    toc.innerHTML=heads.map((h,i)=>{if(!h.id)h.id=`section-${i+1}`;return `<a href="#${h.id}">${LitalkBlog.escapeHtml(h.textContent)}</a>`;}).join('');
  }
  ...
  function renderCurrentPost(){
    ...
    document.title=`${title} — LITALK Education`;
    setText('post-title',title);setText('post-category',post.category||'Article');setText('post-excerpt',excerpt);setText('post-date',LitalkBlog.fmtDate(post.publishedAt));
    const author=post.authorName||post.author||'';setText('post-author',author);if($('post-author'))$('post-author').hidden=!author;
    if($('post-excerpt'))$('post-excerpt').hidden=!excerpt;
    $('post-cover-wrap').innerHTML=media(post,'post-editorial__cover');$('post-cover-wrap').hidden=!$('post-cover-wrap').innerHTML;
    $('post-content').innerHTML=LitalkBlog.mdToHtml(body);
    const minutes=readingMinutes(body);
    setText('reading-time',LitalkBlog.lang()==='th'?`อ่านประมาณ ${minutes} นาที`:`${minutes} min read`);
    buildToc();
    if($('post-more-grid'))$('post-more-grid').innerHTML=relatedPosts.map(recommendCard).join('');
    if($('post-more'))$('post-more').hidden=!relatedPosts.length;
  }

  function bindShare(){
    if(shareBound)return;
    shareBound=true;
    const url=canonicalUrl(),copy=$('copy-link'),share=$('native-share');
    if(copy)copy.addEventListener('click',async()=>{
      const ok=await copyText(url);
      copy.setAttribute('aria-label',ok?(LitalkBlog.lang()==='th'?'คัดลอกลิงก์แล้ว':'Link copied'):(LitalkBlog.lang()==='th'?'คัดลอกลิงก์ไม่สำเร็จ':'Could not copy link'));
      if(ok){copy.innerHTML='<i class="fas fa-check"></i>';setTimeout(()=>copy.innerHTML='<i class="fas fa-link"></i>',1500);}
    });
    if(share&&navigator.share){
      share.hidden=false;
      share.addEventListener('click',()=>{
        if(!currentPost)return;
        navigator.share({title:LitalkBlog.pick(currentPost,'title'),text:LitalkBlog.pick(currentPost,'excerpt'),url}).catch(()=>{});
      });
    }
  }
  ...
  async function load(){
    ...
    try{
      const post=await LitalkBlog.fetchPost(slug);
      if(!post){$('post-content').innerHTML='<p>Article not found.</p>';return;}
      currentPost=post;
      const url=canonicalUrl();const canonical=document.querySelector('link[rel="canonical"]');if(canonical)canonical.href=url;
      const posts=await LitalkBlog.fetchPosts();relatedPosts=posts.filter(p=>p.slug!==slug).slice(0,3);
      renderCurrentPost();
      bindShare();
  ...
  document.addEventListener('litalk:langchange',renderCurrentPost);
